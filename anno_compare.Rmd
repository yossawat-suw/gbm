...
source_rmd("./../script/parameter.Rmd")
...
geom_bar() +
geom_text(stat='count', aes(label=..count..), vjust=-1) + ylim(0,1000) +
...
ggplot(each.meta.df, aes(x = radiation, y = per, fill = consensus))+
  ...
  geom_text(aes(label = paste(per,"%")), position = position_stack(vjust = 0.5),size = 1.5) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
```

```{r}
ggplot(each.meta.df, aes(x = consensus, y = per, fill = radiation, colour = radiation)) + geom_bar(stat = "identity", position = "dodge",width = 0.5) + 
...
ggplot(res.trans[res.trans$radiation=="control",], aes(x = radiation, y = per, fill = subtype))+
  ...
  geom_bar(stat = "identity") + 
  theme(axis.text.y = element_blank(),axis.ticks.y = element_blank()) +
  geom_text(aes(label = paste(per,"%")), position = position_stack(vjust = 0.5),size = 1.5) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + labs(title = "control")
```
...
res.trans.2.diff <- res.trans.2[grep("radiated",rownames(res.trans.2)),] - res.trans.2[grep("control",rownames(res.trans.2)),] 
...
ggplot(res.trans.all, aes(x = radiation, y = per, fill = subtype))+
  geom_bar(stat = "identity") + 
  geom_text(aes(label = paste(per,"%")), position = position_stack(vjust = 0.5),size = 1.5) + labs(subtitle = "all each donor no weight on donor_size")
```
...
for (i in 1:nrow(test10)) {
  donor.id.rep <- data.frame(donor_id = rep(test10[i,"donor_id",drop =TRUE],times = 5),radiation = rep(test10[i,"radiation",drop =TRUE],times = 5))
...
ggplot(res.trans, aes(x = radiation, y = per, fill = subtype))+
  ...
  geom_bar(stat = "identity") + 
  theme(axis.text.y = element_blank(),axis.ticks.y = element_blank()) +
  geom_text(aes(label = paste(per,"%")), position = position_stack(vjust = 0.5),size = 1.5) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + labs(title = "3in4")
```
...
ggplot(res.trans.all, aes(x = radiation, y = per, fill = subtype))+
  geom_bar(stat = "identity") + 
  geom_text(aes(label = paste(per,"%")), position = position_stack(vjust = 0.5),size = 1.5) + labs(title = "3in4",subtitle = "all each donor no weight on donor_size")
```
...
ggplot(all.count.consensus, aes(x=merge)) +
  geom_bar() +theme(axis.text.x = element_text(angle = 90,size = 5))
...
control <- data.frame(res.trans.2[grepl("control",rownames(res.trans.2)),])
colnames(control) <- c("control_AC","control_MES","control_NPC","control_OPC")
rownames(control) <- unique(res.trans$donor_id)

radiated <- data.frame(res.trans.2[grepl("radiated",rownames(res.trans.2)),])
...
cls.radiated <- hclust(as.dist(radiated.dis))
...
res.trans <- res.trans %>%
  mutate(radiation = fct_relevel(radiation, 
            "radiated", "control"))
res.trans <- res.trans %>%
  mutate(donor_id = fct_relevel(donor_id, 
...
p.subtype.control <- ggplot(res.trans[res.trans$radiation=="control",], aes(x = radiation, y = per, fill = subtype)) +
  ...
  geom_bar(stat = "identity") + 
  theme(axis.text.y = element_blank(),axis.ticks.y = element_blank()) +
  geom_text(aes(label = paste(per,"%")), position = position_stack(vjust = 0.5),size = 1.5) + theme(axis.text.y= element_blank(),strip.text = element_text(size = 5),panel.spacing = unit(0.01, "cm"),axis.title = element_blank(),axis.text.x = element_blank(),axis.ticks.x = element_blank(),legend.position = "bottom",legend.key.size = unit(0.2, 'cm'), #change legend key size
...
diff.subtype <- ggplot(res.trans.2.diff.long, aes(x = subtype, y = change, fill = subtype)) +
  ...
  geom_bar(stat = "identity") +
  coord_flip() +
  theme(axis.text.y = element_blank(),axis.ticks.y = element_blank(),strip.text = element_text(size = 5),panel.spacing = unit(0.2, "cm"),axis.title = element_blank(),axis.text.x = element_blank(),axis.ticks.x = element_blank(),legend.position = "bottom",legend.key.size = unit(0.1, 'cm'), #change legend key size
        legend.title = element_blank(), #change legend title font size
...
grid.arrange(dendro,p.subtype.control,p.subtype.radiated,gb, layout_matrix = rbind( c(1,2,2,3,3,4,4,4,4,4),
                                                                     c(1,2,2,3,3,4,4,4,4,4)),
...
#Try using from PCA
library(ggfortify)
pca.input <- list(list("control & radiated",c("control_AC" ,"control_MES", "control_NPC" ,"control_OPC","radiated_AC" ,"radiated_MES", "radiated_NPC" ,"radiated_OPC")),
...
```
```{r}
donor.meta.diff.class <- subset(donor.meta, select = -c(Runs,control_AC,control_MES,
...
```{r}
#Control
donor.meta.control.class <- subset(donor.meta, select = -c(Runs,diff_AC,diff_MES,
diff_NPC,diff_OPC))
