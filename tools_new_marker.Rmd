...
sigs <- c("6metamodules","4metamodules_new","4metamodules_new_50","MESImm","3metamodules_new_50","4metamodules_new_50_mes")
sig <- sigs[6]
sig
...
gbm.list.res <- mclapply(gbm.list,mc.cores = 4, FUN = function(x){
  ...
  exp[] = normalize.quantiles(as.matrix(exp))
  results = SCINA(exp, signatures.scina, rm_overlap=FALSE, allow_unknown=FALSE)
  return(results)
})
```
