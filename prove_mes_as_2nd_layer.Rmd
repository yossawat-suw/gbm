...
res.list <- lapply(gbm.list, FUN = function(gbm.each) {
  ...
  p_ucell <- ggscatter(
    data_long_ucell, x = "MES.new_UCell", y = "measurement",
  ...
  p_seurat <- ggscatter(
     data_long_seurat, x = "mes4", y = "measurement",
    color = "condition", palette = "jco",
    add = "reg.line"
    ) +
    facet_wrap(~condition) +
...
mm_plot <- ggplot(gbm.meta.ucell_long, aes(x = MES.new_UCell, y = measurement, colour = condition)) +
      ...
      theme(legend.position = "none",
            panel.spacing = unit(0, "cm"))  # adding space between panels
...
(ggplot(pred.mm) + 
   ...
   geom_point(data = gbm.meta.ucell_long.ac,                      # adding the raw data (scaled values)
...
(ggplot(pred.mm) + 
   ...
   geom_ribbon(aes(x = x, ymin = predicted - std.error, ymax = predicted + std.error), 
               fill = "lightgrey", alpha = 0.5) +  # error band
   geom_point(data = gbm.meta.ucell_long.npc,                      # adding the raw data (scaled values)
              aes(x = MES.new_UCell, y = measurement, colour = donor_radiation),alpha = 0.1, size = 0.1) + 
   labs(x = "MES", y = "-log(npc)", 
        title = "") + 
   theme_minimal()
)
...
ggplot(pred.mm) + 
   ...
   geom_ribbon(aes(x = x, ymin = predicted - std.error, ymax = predicted + std.error), 
               fill = "lightgrey", alpha = 0.5) +  # error band
   geom_point(data = gbm.meta.ucell_long.opc,                      # adding the raw data (scaled values)
              aes(x = MES.new_UCell, y = measurement, colour = donor_radiation),alpha = 0.1, size = 0.1) + 
   labs(x = "MES", y = "-log(OPC)", 
        title = "") + 
   theme_minimal()
