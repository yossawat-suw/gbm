...
```{r}
sen_score <- fread(file = "output/score/sen_score.csv",data.table = FALSE)
sen_score  <- column_to_rownames(sen_score,var = "V1")
```

```{r}
anno.all.short <- fread(file = paste("output/consensus/annotation_short",merge,run,unknown,".csv",sep = "_"))
...
```{r}
#Find the median 
sen_score_average <- anno.senesence %>% group_by(consensus,donor_id,radiation) %>% summarise(median_sen_score = median(sen_score),
                                                                                             mean_sen_score = mean(sen_score)) %>% ungroup()
``` 

```{r}
sen_score_average %>% filter(radiation == "control") %>%
  ggplot(data = ., aes(x=donor_id,y= median_sen_score,fill = consensus,color = consensus,group = consensus)) +
  geom_point() + geom_line() + theme_classic()
```
```{r}
sen_score_average %>% filter(radiation == "control") %>%
  ggplot(data = ., aes(x=donor_id,y= mean_sen_score,fill = consensus,color = consensus,group = consensus)) +
  geom_point() + geom_line() + theme_classic()
```
...
sen_score_average  %>%
  ggplot(data = ., aes(x=donor_id,y= mean_sen_score,fill = consensus,color = consensus,group = consensus)) +
  geom_point() + geom_line() + theme_classic() + facet_wrap(~radiation)
```
...
```
```{r}
table(annotation$radiation,annotation$donor_id,annotation$consensus)
```
```{r}
table(annotation$radiation,annotation$consensus)
```
```{r}
table(annotation$radiation,annotation$consensus,annotation$donor_id)
```
