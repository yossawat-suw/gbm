...
if (length(chosen_celltypes > 1)) {
     Idents(neftel.smt) <- "celltype_merge"; neftel.smt <- subset(neftel.smt,idents = chosen_celltypes);
} else {
 neftel.smt$celltype_merge <- as.character(neftel.smt$celltype_merge); neftel.smt@meta.data[!(neftel.smt$celltype_merge %in% chosen_celltypes  ),"celltype_merge"] <- "not_labeled"; neftel.smt$celltype_merge <- as.factor(neftel.smt$celltype_merge);
}
...
gbm.list.res <- mclapply(X = gbm.list[5], mc.cores = n.cores, FUN = function(x) {
