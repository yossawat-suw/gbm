...
anno <- bind_cols(sc.type, scina ) #marker based no sc.sorter (for the one with only one group of cell)
...
```{r}
ggsave(paste0("figure/sankey_tools_",pick,"_",merge,"_",run,"_",unknown,".png"))
```
...
col_selected <- c(setdiff(colnames(anno_score),"unknown"),"unknown")

anno_score <- anno_score[,..col_selected]
...
if (ncol(anno_score) == 1) {
  anno_score[,paste0("Rank_",possible_cellstate)] <- 1
} else {
...
table(anno.all.short$consensus)
```
