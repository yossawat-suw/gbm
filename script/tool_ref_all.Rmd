...
gbm.list.res <- mclapply(X = gbm.list, mc.cores = n.cores, FUN = function(x) {
  ...
  #the edited does nothing more except that i export more information of unknown cell, but the result is not useful fornow because i didnt correctly export the right score but i dont have time to recheck it yet 
...
#system.time(gbm.list.res <- lapply(X = gbm.list,  FUN = function(x)  {
  ...
  x.default <- scmapCell2Cluster(x, list(as.character(neftel.smt.sce@colData[,celltype])),w = 3,threshold = 0.5) #default
   x.adjust <- scmapCell2Cluster(x, list(as.character(neftel.smt.sce@colData[,celltype])),w = 1,threshold = 0) #adjust to extract most
