...
```{r}
saveRDS(gbm,paste0("./../output/","seurat_gbm_qc"),compress = FALSE)
```

```{r}
gbm <- CellCycleScoring(object = gbm, g2m.features = cc.genes$g2m.genes,
    s.features = cc.genes$s.genes)
```
```{r}
VlnPlot(gbm, features = c("S.Score", "G2M.Score"), group.by = "orig.ident",
    ncol = 4, pt.size = 0.1)
```
...
gbm = ScaleData(gbm, vars.to.regress = c("nFeature_RNA", "percent.mt"),
    verbose = F)
...
```{r}
neftel.smrt <- SingleCellExperiment(assays = list(logcounts = as.matrix(smrt_mtx)),
                                    colData = meta)
```
