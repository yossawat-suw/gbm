...
```{r}
anno_score$consensus <- as.character(anno_score$consensus)
possible_cellstate <- unique(unname(unlist(anno_score[,c("stochastic_consensus","consensus_with_stochastic_of_tie","consensus")])))
...
plot(x = anno_score$unknown,y = anno_score$confidence)
plot(x = anno_score$unknown,y = anno_score$agreement)
```

```{r}
# Assuming you have a predefined set of levels for 'consensus'
anno_score_meta$consensus <- factor(anno_score_meta$consensus,levels = possible_cellstate_with_unknown)
# Your original code with complete


count_consensus <- anno_score_meta %>%
  group_by(radiation, donor_id, consensus) %>%
  summarise(count_consensus = n()) %>%
  complete(consensus, fill = list(count_consensus = 0))
...
mean_score <- anno_score_meta %>%
  ...
  summarise(confidence = mean(confidence),
            ...
            unknown_prop = mean(unknown),
            cell_number = n())

mean_score
```
```{r}
plot(x = mean_score$agreement,y = mean_score$confidence)
plot(x = mean_score$unknown_prop,y = mean_score$confidence)
plot(x = mean_score$unknown_prop,y = mean_score$agreement)
```
```{r}
radiations <- c("control","radiated")
radiation <- radiations[1]
ggplot(mean_score[mean_score$radiation == radiation,],
       aes(x=agreement,y = confidence,size = cell_number,color = unknown_prop)) +
  geom_point() +
  labs(title = "mean score",subtitle = radiation)

radiation <- radiations[2]
ggplot(mean_score[mean_score$radiation == radiation,],
       aes(x=agreement,y = confidence,size = cell_number,color = unknown_prop)) +
  geom_point() +
  labs(title = "mean score",subtitle = radiation)
```
```{r}
# Function to create a ggplot with fixed color and size scales
create_plot <- function(radiation) {
  ggplot(mean_score[mean_score$radiation == radiation,],
         aes(x = agreement, y = confidence, size = cell_number, color = unknown_prop)) +
    geom_point() +
    labs(title = "Mean Score", subtitle = radiation) +
...
# Create plots for each radiation level
plot_control <- create_plot("control")
plot_radiated <- create_plot("radiated")

# # Display the plots side by side
# library(gridExtra)
#grid.arrange(plot_control, plot_radiated, nrow = 2)
plot_control
plot_radiated
...
scatterplot3d(mean_score$agreement, mean_score$confidence, mean_score$unknown_prop, 
              color = "blue", main = "3D Scatter Plot",
              xlab = "agreement", ylab = "confidence", zlab = "unknown_prop",
