...
prob_all <- data.frame(matrix(nrow = unique_tool_donor_radiation, ncol = 0))
for (i in 2:ncol(anno_cellstate_meta_control)) {
  anno_cellstate_meta_eachtool <- anno_cellstate_meta_control[,c(1,i)] 
  ...
  for (q in unique(anno_cellstate_meta_eachtool$donor_radiation)) {
    ...
    prob_tool[,2] <- prob_tool[,2]/nrow(anno_cellstate_meta_eachtool_eachdonor)
    prob_tool_alldonor <- rbind(prob_tool_alldonor,prob_tool)
    prob_tool_alldonor <- prob_tool_alldonor %>% arrange(donor_radiation,cellstate)
...
```{r}
#calculate clr
prob_all_list_imputed_clr <- lapply(prob_all_list_imputed, CLR,weight=weight)
prob_all_nounknown_list_imputed_clr <- lapply(prob_all_nounknown_list_imputed, CLR,weight=weight)
```


```{r}
# lapply(prob_all_nounknown_list_imputed, function(x) {
#   print(sum(x == 0) > 0) 
# })
```
...
lrv_list <- lapply(prob_all_list_imputed_clr, LR.VAR,weight=weight)
lrv_nounknown_list <- lapply(prob_all_nounknown_list_imputed_clr, LR.VAR,weight=weight)
```
...
tool <- "tool"
proportion <- "proportion"
gathercols <- colnames(prob_all)[!colnames(prob_all) %in% c("cellstate","donor_radiation")]

prob_all_long <- gather(prob_all, tool, proportion, gathercols)
prob_all_long
prob_all_long$tool <- factor(prob_all_long$tool,levels = rownames(metric))

prob_all_long$donor_radiation <- factor(prob_all_long$donor_radiation,level = donor_radiation_bysize)


plot.prop <- ggplot(prob_all_long, aes(fill=cellstate, y=proportion, x=donor_radiation)) + 
    geom_bar(position="fill", stat="identity") +
  facet_wrap(~tool) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,size = 3)) +
  
  labs(title = "Cellstate annotaion",
       subtitle = paste0("analysed group: ","both"),
       caption = paste0(
                         "metric = ",chosen_metric,"\n",
                         "criteria = ",paste(score_name[pick],collapse = ": "),";  ",
                         "weight = ",paste(weights[pick],collapse = ":")))
