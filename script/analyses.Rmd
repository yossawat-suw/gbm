...
vc_common <- fread("output/consensus/annotation_short_vc_common_each__.csv")
...
setnames(vc_common,c("rn","consensus"),c("cell_id_Yossawat","vc_common"))
...
annotation <-  merge(annotation,vc_common[,.(cell_id_Yossawat,vc_common)],by = "cell_id_Yossawat")
...
```
```{r}
vc_common <- dcast(annotation,formula = donor_id + radiation ~ vc_common)


vc_common <- cbind(vc_common[,1:2],t(apply(vc_common[,3:4],1,FUN = function(x) x / sum(x))))

vc_common_control <- vc_common[radiation == "control", -"radiation"] %>% tibble::column_to_rownames(var = "donor_id")

vc_common_radiated <- vc_common[radiation == "radiated", -"radiation"] %>% tibble::column_to_rownames(var = "donor_id")
...
#try transverse but with weight
```{r}
library(tidyr)
sen_score <- annotation %>% group_by(consensus,radiation,donor_id) %>%
  summarise(
  sen_score = mean(sen_score),
) %>% 
  group_by(consensus,radiation) %>%
  summarise(
    sen_score = mean(sen_score)
...
sen_score <- sen_score %>% spread(key = radiation,value = sen_score)
setnames(sen_score, c("control","radiated"),c("Control","Radiated"))
...
#RSI
rsi <- annotation %>% group_by(consensus,radiation,donor_id) %>%
  summarise(
  RSI = mean(RSI),
) %>% 
  group_by(consensus,radiation) %>%
...
rsi <- rsi %>% spread(key = radiation,value = RSI)
setnames(rsi, c("control","radiated"),c("Control","Radiated"))
...
#p12 (p11)
#RSI
p11 <- annotation %>% group_by(consensus,radiation,donor_id) %>%
  summarise(
  P12 = mean(P12),
) %>% 
  group_by(consensus,radiation) %>%
...
p11 <- p11 %>% spread(key = radiation,value = P12)
setnames(p11, c("control","radiated"),c("Control","Radiated"))

p11 <- p11 %>% tibble::column_to_rownames(var = "consensus") 
```

```{r}
pathway <- dcast(annotation,formula = consensus + radiation + donor_id ~ pathway)


pathway <- cbind(pathway[,1:3],t(apply(pathway[,4:7],1,FUN = function(x) x / sum(x))))

pathway <- pathway %>% group_by(consensus,radiation) %>%
summarise(across(c("GPM", "MTC","NEU","PPR"), ~ mean(.x, na.rm = TRUE)))

# pathway <- pathway %>% group_by(consensus,radiation) %>%
#   summarise(across(GPM:unknown, ~ mean(.x, na.rm = TRUE)))
...
```
```{r}
neurodevelopmental <- dcast(annotation,formula = consensus + radiation + donor_id ~ neurodevelopmental)



neurodevelopmental <- cbind(neurodevelopmental[,1:3],t(apply(neurodevelopmental[,4:8],1,FUN = function(x) x / sum(x))))


neurodevelopmental <- neurodevelopmental %>% group_by(consensus,radiation) %>%
  summarise(across(Astro:Progenitor, ~ mean(.x, na.rm = TRUE)))

setDT(neurodevelopmental)
...
```{r}
png( "figure/hm_4cellstates_transpose_weighted.png",width=20,height=12,units="cm",res=300)
draw(hm_trans, merge_legend = TRUE, heatmap_legend_side = "right", 
    annotation_legend_side = "right")
dev.off()
```

#try transverse but with weight + min-max normalization on score
```{r}
library(tidyr)
sen_score <- annotation

#min-max norm
sen_score$sen_score <- scale(sen_score$sen_score, center = min(sen_score$sen_score), scale = max(sen_score$sen_score) - min(sen_score$sen_score))
sen_score <- sen_score %>% group_by(consensus,radiation,donor_id) %>%
  summarise(
  sen_score = mean(sen_score),
) %>% 
  group_by(consensus,radiation) %>%
...
rsi <- annotation 
#min-max norm
rsi$RSI <- scale(rsi$RSI, center = min(rsi$RSI), scale = max(rsi$RSI) - min(rsi$RSI))
rsi <- rsi %>% group_by(consensus,radiation,donor_id) %>%
  summarise(
  RSI = mean(RSI),
) %>% 
  group_by(consensus,radiation) %>%
...
p11 <- annotation 
#min-max norm
p11$P12 <- scale(p11$P12, center = min(p11$P12), scale = max(p11$P12) - min(p11$P12))

p11 <- p11 %>% group_by(consensus,radiation,donor_id) %>%
  summarise(
  P12 = mean(P12),
) %>% 
  group_by(consensus,radiation) %>%
...
```{r}
png( "figure/hm_4cellstates_transpose_norm_weighted.png",width=20,height=12,units="cm",res=300)
draw(hm_trans, merge_legend = TRUE, heatmap_legend_side = "right", 
    annotation_legend_side = "right")
dev.off()
```
