...
```{r}
print("load neftel")
reference.original <- readRDS("output/seurat_objects/smrt_mal.rds")

if (length(chosen_celltypes) > 1) {
     Idents(reference.original) <- celltype; reference.original <- subset(reference.original,idents = chosen_celltypes); reference.original@meta.data[,celltype] <- as.character(reference.original@meta.data[,celltype]);
} else {
 reference.original@meta.data[,celltype] <- as.character(reference.original@meta.data[,celltype]); reference.original@meta.data[!(reference.original@meta.data[,celltype] %in% chosen_celltypes  ),celltype] <- "not_labeled"; reference.original@meta.data[,celltype] <- reference.original@meta.data[,celltype];
}
...
<!--   seurat_object = reference,        # SeuratV3 object -->
...
```{r}
reference <- reference.original
reference_gem <- as.matrix(reference@assays$norm@data)
...
```{r}
# reference <- readRDS("output/seurat_objects/smrt_mal.rds"); reference <- subset(reference,idents = chosen_celltypes);
reference <- reference.original
...
```{r}
#reference <- readRDS("output/seurat_objects/smrt_mal.rds")
...
<!-- #reference.scn <- readRDS("output/seurat_objects/smrt_mal.rds") -->
...
<!-- <!-- exprsMat_neftel <- reference@assays$RNA@data --> -->

<!-- <!-- neftel_celltype_merge <- reference$celltype_merge --> -->
<!-- <!-- rm(reference) --> -->
<!-- <!-- ``` --> -->
<!-- <!-- ```{r} --> -->
...
<!-- <!-- rm(res.df,neftel_celltype_merge,exprsMat_neftel,reference) --> -->
<!-- <!-- gc() --> -->
<!-- <!-- ``` --> -->
